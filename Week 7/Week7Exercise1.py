...
def calculate_discount(category, tier):
    """Calculate total discount percentage based on category and tier."""
    category_discounts = {
        'Electronics': 10,
        'Clothing': 15,
        'Books': 5,
    ...
    # Use .get() to safely retrieve values with a default of 0
...
def process_products(input_file, output_file):
    """Read products, calculate prices, and generate report."""
    try:
        products = []
        total_discount = 0
        ...
        with open(input_file, 'r') as f:
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    # Unpack parts and clean whitespace
                    name, price_str, category, tier = [p.strip() for p in parts]
